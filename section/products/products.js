...
function createProductElement(productId, productName, productDesc, productPrice){
    var col = '<div class="col-4 product-container"><img src="test.jpg" alt=""><form action="../product/product.php" method="POST"><div class="product-info-container"><input type="hidden" name="product_id" value="' + productId + '"><p class="product-title">' + productName + '</p><p class="product-desc">' + productDesc + '</p><p class="product-price">$' + productPrice + '</p><div class="product-buttons-container"><button type="button" onclick="addToCart(1,' + productId + ', ' + productPrice + ')">Agregar al carrito</button><button type="submit">Más información</button></div></div></form></div>';
    return col;
};

function addToCart(cantidad, productId, productPrice){
    $.ajax({
        type: "POST",
        ...
        data: {
            ...
            precioCarrito: Number(productPrice)
